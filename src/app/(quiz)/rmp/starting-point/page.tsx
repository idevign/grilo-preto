...
function getResults(basePath: string) {
  return {
    guided: {
      label: "RMP⁺ Guided",
      paragraph:
...
type Result = { label: string; paragraph: string; href: string };
...
export default function StartingPoint() {
  ...
  const results = getResults(basePath);
  ...
  const resultKey = answers.length === questions.length ? computeResult(answers) : "guided";
    ...
    <main style={{ height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      >
        <div style={{ width: "100%", maxWidth: "560px", padding: "0 1.5rem" }}>
          ...
          {screen === "result" && (
            <ResultScreen result={results[resultKey]} onRestart={handleRestart} />
          )}
        </div>
      </div>

      {/* Bottom step counter — always rendered to preserve centering */}
      <div
        style={{
          flexShrink: 0,
      ...
      >
        ...
        <div style={{ width: "80px", height: "1px", backgroundColor: "var(--color-subtle)", position: "relative" }}>
          <div
            style={{
              position: "absolute",
              left: 0,
      ...
      <style>{`
        ...
        .quiz-option:hover { border-color: var(--color-mid); }
